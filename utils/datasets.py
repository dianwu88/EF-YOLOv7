...
def imadjust(img, out_range=[0, 1], gamma=1):
    ...
    img_out = (((img_out - low_in) / (high_in - low_in)) ** gamma) * (high_out - low_out) * 255 + low_out

    return img_out

def image_enhancement(img0):
    def replaceZeroes(data):
        min_nonzero = min(data[np.nonzero(data)])
        data[data == 0] = min_nonzero
        return data

    def agcwd(image, w ,alpha):
        ...
        l_intensity = np.array([255 * (e / 255) ** ((1 - w_img_cdf[e]) * (alpha ** (abs(128 - mean_img) / 128))) for e in l_intensity], dtype=np.uint8)
    ...
    def detail_enhancement(image):
        ...
        gg = np.array(img[:,:],dtype=np.float64)
        for i in range(0, height):
            for j in range(0, width):
                gg[i,j] = (img[i,j] - mean_img) / (480 * 640 - 1)


        summm = np.sum(gg)
        Gaus = np.arange(0, 256)
        Gaus = np.array([1 / ( 2 * pi * var_img) * exp( -(e ** 2) / (2 * var_img)) * summm for e in Gaus],dtype=np.uint8 )
        Gaus_p = np.array([img] ,dtype=np.float64)
        for i in range(0, height):
            for j in range(0, width):
                intensity = img[i,j]
                Gaus_p[i,j] = Gaus[intensity]
        Gaus_p = set_value_channel(image, Gaus_p) if is_colorful else Gaus_p
        return Gaus_p
    ...
    class AGC:
        ...
        def gamma_correction(self, img):
            ...
            img_hsv_PIL = Image.fromarray(img_hsv, 'HSV')
        ...
        def CLAHE(self, img, clipLimit=2.0, tileGridSize_h=8, tileGridSize_w=8):
    ...
    enhance = AGC()
    img0 = enhance.gamma_correction(img0)
    ...
    Vout = agcwd(V , 0.1 ,0.6)
    img1 = cv2.merge([H, S, Vout])
    img2 = multiScaleSharpen(img1 , 5)
