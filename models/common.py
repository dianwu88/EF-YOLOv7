...
# from models.att import CrissCrossAttention
# from models.att import RFAconv
# from models.att import DiffAttentionModule
# from models.att import CBAMLayer
# from models.att import Transcam
...
class Conv_se(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.se = CBAMLayer(c2)

    def forward(self, x):
        ff = self.act(self.bn(self.conv(x)))
        return ff
...
class Conv_att(nn.Module):
    ...
    def forward(self, x):
        ff = self.att(self.act(self.bn(self.conv(x))))
        return ff
...
class Conv_g(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.conv1 = nn.Conv2d(c1*4, c2 , k, s)
    ...
    def forward(self, x):
        x = self.conv(x)
        max5 = self.maxpool5(x)
        max9 = self.maxpool9(max5)
        max13 = self.maxpool13(max9)
        ff = torch.cat([x,max5,max9,max13],1)
        ff = self.conv1(ff)
        ff = self.pconv(ff)
        return ff
...
class Pconv(nn.Module):
    def __init__(self, dim, out, n_div=4, forward='split_cat'):
        super().__init__()
        self.dim_conv3 = dim // n_div
        self.dim_untouched = dim - self.dim_conv3
        self.partial_conv3 = nn.Conv2d(self.dim_conv3, self.dim_conv3, 3, 1, 1, bias=False)
        self.conv = Conv(dim,out,k=1)
...
class DenseLayer(nn.Module):
    def __init__(self, num_input_features, bn_size=4, drop_rate=0):
        super(DenseLayer, self).__init__()
        self.drop_rate = drop_rate
        self.growth_rate = num_input_features // 2
        self.dense_layer = nn.Sequential(
            nn.BatchNorm2d(num_input_features),
            nn.LeakyReLU(0.1),
            nn.Conv2d(in_channels=num_input_features, out_channels=bn_size * self.growth_rate, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(bn_size * self.growth_rate),
            nn.LeakyReLU(0.1),
            nn.Conv2d(in_channels=bn_size * self.growth_rate, out_channels=self.growth_rate, kernel_size=3, stride=1, padding=1, bias=False)
        )
        self.conv1 = nn.Conv2d(num_input_features+self.growth_rate,num_input_features,kernel_size=1,stride=1,padding=0)
...
class edm(nn.Module):
    def __init__(self, dim,out):
        ...
        kernel_lap =[[0, 1, 0],
                    [1, -4, 1],
                    [0, 1, 0]]
        self.conv1 = nn.Conv2d(dim ,1 ,kernel_size=3)
        self.conv2 = nn.Conv2d(1 ,dim ,kernel_size=3 ,padding=2)
        self.relu = nn.ReLU()
        kernel_h = torch.FloatTensor(kernel_h).unsqueeze(0).unsqueeze(0)
        kernel_v = torch.FloatTensor(kernel_v).unsqueeze(0).unsqueeze(0)
        kernel_lap = torch.FloatTensor(kernel_lap).unsqueeze(0).unsqueeze(0)
        self.weight_h = nn.Parameter(data=kernel_h, requires_grad=False)
        self.weight_v = nn.Parameter(data=kernel_v, requires_grad=False)
        self.weight_lap = nn.Parameter(data=kernel_lap, requires_grad=False)
        self.gaussian_layer = get_gaussian_kernel()
    def forward(self, x):
        x = self.conv1(x)
        x_v = F.conv2d(x, self.weight_v, padding=1)
        x_h = F.conv2d(x, self.weight_h, padding=1)
        gs = self.gaussian_layer(x)
        F_lap = F.conv2d(gs,self.weight_lap,padding=1)
        F_sobel = torch.sqrt(torch.pow(x_v, 2) + torch.pow(x_h, 2) + 1e-6)
        out = F_sobel + 0.4 * (F_lap + F_sobel)
...
class AOD_net(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.conv0 = Conv1(3, 1, 3, 1)
        self.conv1 = Conv1(3, 8, 3, 1)
        self.conv2 = Conv1(8, 16, 3, 1)
        self.conv3 = Conv1(24, 16, 5, 1)
        self.conv4 = Conv1(32, 16, 7, 1)
        self.conv5 = Conv1(56, 8, 3, 1)
    ...
    def forward(self, x):
        ...
        x0 = self.conv0(x)
        x1 = self.lrelu(self.conv1(x))#8
        x2 = self.lrelu(self.conv2(x1))#16
        cat1 = torch.cat((x1, x2), 1)#24
        x3 = self.lrelu(self.conv3(cat1))#16
        cat2 = torch.cat((x2, x3), 1)#32
        x4 = self.lrelu(self.conv4(cat2))#16
...
class RepConv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        bb = self.act(self.rbr_dense(inputs) + self.rbr_1x1(inputs) + id_out)
