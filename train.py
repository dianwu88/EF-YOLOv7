...
# python detect.py --source D:\yolov7-main\data\cvc --weights D:\yolov7-main\runs\train\exp6\weights\best.pt

# python train.py --batch-size 4 --epoch 10 --data D:\yolov7-main\data\infrared.yaml --weights D:\yolov7-main\weights\yolov7.pt
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='D:/yolov7-main/weights/yolov7.pt', help='initial weights path')#网络权重，可根据需求设置不同尺寸大小的权重
    parser.add_argument('--cfg', type=str, default='cfg/training/yolov7.yaml', help='model.yaml path')#指定模型配置文件路径的；配置文件里面指定了一些参数信息和backbone的结构信息。
    parser.add_argument('--data', type=str, default='data/infrared.yaml', help='data.yaml path')#数据集的种类以及每个种类对应的名称
    parser.add_argument('--hyp', type=str, default='data/hyp.scratch.p5.yaml', help='hyperparameters path')#包含了超参数的信息
    parser.add_argument('--epochs', type=int, default=100)                                                       #训练迭代数
    parser.add_argument('--batch-size', type=int, default=6, help='total batch size for all GPUs')#每批次的输入数据量；default=-1将时自动调节batchsize大小，代表一次输入16张图片进行训练。
    parser.add_argument('--img-size', nargs='+', type=int, default=[640, 640], help='[train, test] image sizes')#训练集和测试集输入的图片像素大小
    parser.add_argument('--rect', action='store_true', help='rectangular training')#是否采用矩阵推理的方式去训练模型，采用矩阵推理的话就不要求送入的训练的图片是正方形了
    parser.add_argument('--resume', nargs='?', const=True, default=False, help='resume most recent training')#断点训练，如需要，将default设置为True,--resume (模型保存路径)
    parser.add_argument('--nosave', action='store_true', help='only save final checkpoint')#是否保存最后一轮的pt文件，默认保存的是best.pt和last.pt
    parser.add_argument('--notest', action='store_true', help='only test final epoch')#平时的训练中在每一轮训练结束的时候都会计算mAP值，如果开始此参数，那么只在最后一轮计算mAP值。
    parser.add_argument('--noautoanchor', action='store_true', help='disable autoanchor check')#是否禁用自动锚框
    parser.add_argument('--evolve', action='store_true', help='evolve hyperparameters')#超参数进化
    ...
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')#训练模型的设备
    ...
    parser.add_argument('--workers', type=int, default=8, help='maximum number of dataloader workers')#指数据装载时cpu所使用的线程数，默认为8。与batch_size不同，消耗cpu
    ...
    parser.add_argument('--quad', action='store_true', help='quad dataloader')#生效后可以在比前面 “--img-size” 部分设置的训练测试数据集更大的数据集上训练。
    parser.add_argument('--linear-lr', action='store_true', help='linear LR')#用于对学习速率进行调整
    parser.add_argument('--label-smoothing', type=float, default=0.0, help='Label smoothing epsilon')#用于对标签进行平滑处理。作用是防止在分类算法中过拟合情况的产生。
